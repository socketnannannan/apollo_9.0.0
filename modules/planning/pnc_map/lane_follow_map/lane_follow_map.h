...
class LaneFollowMap : public PncMapBase {
 ...
 private:
  struct RouteIndex {  // 定义存储lane数据，其中index为当前lane_segment 的 {road_index, passage_index, lane_index}的三元组。
   ...
   */
  struct WaypointIndex {  // 定义存储waypoint，以及该waypoint在route_indices_中的序号index。
