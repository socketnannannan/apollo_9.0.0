...
// 更新主车在当前路线上的行驶范围，并维护一个未经过的车道段ID集合
...
bool LaneFollowMap::UpdateVehicleState(const VehicleState &vehicle_state) {
  ...
  if (!GetNearestPointFromRouting(vehicle_state, &adc_waypoint_)) { // 调用GetNearestPointFromRouting成员函数从routing中查找到最近的道路点adc_waypoint_
    AERROR << "Failed to get waypoint from routing with point: "
           << "(" << vehicle_state.x() << ", " << vehicle_state.y() << ", "
           << vehicle_state.z() << ").";
    return false;
  }
  int route_index = GetWaypointIndex(adc_waypoint_);// 根据adc_waypoint_调用GetWaypointIndex成员函数查找到当前行驶的LaneSegment在route_indices_中的序号route_index，并更新至adc_route_index_
  if (route_index < 0 ||
...
bool LaneFollowMap::UpdatePlanningCommand(
  ...
  all_lane_ids_.clear();
  for (int road_index = 0; road_index < routing.road_size(); ++road_index) { // 用三个for循环分别剥离出routing结果里的road，passage，lane
  ...
  // 重置range_start_，range_end_，adc_route_index_，next_routing_waypoint_index_，
  // 调用UpdateRoutingRange成员函数将routing结果的lane_id存放进range_lane_ids_中。
  range_start_ = 0;
  range_end_ = 0;
  adc_route_index_ = -1;
  next_routing_waypoint_index_ = 0;
...
// 根据adc_waypoint_调用GetWaypointIndex成员函数查找到当前行驶的LaneSegment在route_indices_中的序号route_index，并更新至adc_route_index_
...
// 在ReferenceLineProvider中被调用，用于更新获取当前route_segments，为参考线构建提供数据支持。
...
bool LaneFollowMap::GetRouteSegments( 
  ...
  double look_forward_distance =
      LookForwardDistance(vehicle_state.linear_velocity());// 调用LookForwardDistance，根据车辆当前速度计算前向搜索距离look_forward_distance。后向搜索look_backward_distance由参数控制。
...
// GetRouteSegments的重载函数
bool LaneFollowMap::GetRouteSegments(
    const VehicleState &vehicle_state, const double backward_length,
    const double forward_length,
    std::list<hdmap::RouteSegments> *const route_segments) {
  if (!UpdateVehicleState(vehicle_state)) { // 调用UpdateVehicleState，更新route状态(segment状态)。
  ...
  // 由更新的adc_route_index_,获得当前出车所处的RoadSegment和Passage的序号road_index、passage_index，
  // 根据当前road和passage，调用GetNeighborPassages计算当前车道的所有相邻passage的信息drive_passages
  ...
  // for循环遍历drive_passages进行操作。调用PassageToSegments成员函数，
  // 将passage转换为hdmap::RouteSegments segments，并且计算主车到当前segments的投影点，
  // 从该投影点，按照前向搜索距离look_forward_distance，后向搜索距离look_backward_distance，进行segments的扩展。
  // 从而得到当前passage的segments，并且保存至route_segments。
  for (const int index : drive_passages) {
    ...
    if (!segments.GetProjection(nearest_point, &sl, &segment_waypoint)) { // 获取投影
      ADEBUG << "Failed to get projection from point: "
             << nearest_point.ShortDebugString();
      continue;
    }
    if (index != passage_index) {
      if (!segments.CanDriveFrom(adc_waypoint_)) { // 用于确定车辆是否可以从给定的车道点（LaneWaypoint）出发行驶
...
// 调用GetNearestPointFromRouting成员函数从routing中查找到最近的道路点adc_waypoint_
